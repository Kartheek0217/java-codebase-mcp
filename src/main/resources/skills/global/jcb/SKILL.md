...
- **Project Tasks (TaskManager)** *(Note: All endpoints require `projectId` header)*: `crt-task` (create project task), `get-tasks` (list), `get-task` (status by `id`), `upd-task` (update by `id`), `del-task` (delete by `id`), `upd-task-step` (update step `id`, `stepId`, `status`). These manage project-specific goals and rules.
...
- **Skills & Rules** *(Note: All endpoints require `projectId` header)*: `learn-skill-url` (`url`), `learn-skill-file` (`filePath`), `get-skills` (list), `clear-skills` (), `get-rules` (), `crt-rule` (add), `del-rule` (`id`), `clear-rules` ().
- **Sessions**: `start-session` (create agent session, `projectId` header), `get-session` (metadata, `sessionId`), `lst-sessions` (list, `projectId` header), `crt-session` (new browser, `projectId` header, `browserType`, `headless`, `viewport`), `get-session-state` (HTML/state, `X-View=content`), `browser` (interaction, `X-Action=navigate|screenshot|click|fill|type|select|wait|evaluate|extract-locators`), `close-session` (close context, `sessionId`).
- **Agent Tasks (AgentController)** *(Note: All endpoints require `projectId` header)*: 
  ...
  - **Streaming (SSE - No Envelope)**: `explain-symbol` (req: `symbolId`), `explain-file` (req: `filePath`), `ask-question` (req: `question`), `code-review` (req: `filePath`), `code-refactor` (req: `filePath`), `code-optimise` (req: `filePath`), `web-search` (req: `query`/`url`), `code-commit` (req: `diff`), `java-doc` (req: `filePath`), `junit-test-cases` (req: `filePath`).
- **Codebase Discovery** *(Note: All endpoints require `projectId` header)*: `read-file` (read with symbols), `batch-read-files` (multiple files), `suggest-context` (symbol+content hits), `search-content` (Lucene query), `search-changed-content` (Lucene uncommitted), `search-symbols` (find by name, `type=CLASS|METHOD|FIELD|CONSTRUCTOR`), `find-files` (find paths), `get-session-history` (accessed paths), `summarize-file` (AI file summary), `analyze` (controller endpoint trace), `get-symbol-detail` (details by `id`), `get-call-hierarchy` (call tree by `id`).
